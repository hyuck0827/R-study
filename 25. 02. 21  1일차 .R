...
# 리스트 형태의 데이터 생성
# R에서는 리스트 형태가 python에서는 딕셔너리 형태
# 백터 데이터에서 인덱스(위치) 값을 기준으로 데이터를 출력한다면
# 리스트에서는 위치 대신에 key 값을 기준으로 데이터를 출력
# 데이터의 순서와 상관없이 데이터를 주고 받는다.
# 리스트형태는 데이터의 타입이 여러 개가 동시에 저장 가능
...
list_z = list(name = c('test', 'test2', 'test3'),
              age = c(20,30,40),
              phone = c("01011112222","01033334444","01055556666"))
...
print(df)
# 데이터프레임에서 백터 데이터의 개수가 다르다면 에러 발생
df2 = data.frame(
  name = c("test", "test2","test3"),
  age = c(20,30),
...
# whick문
# 백터데이터에서 조건식에 일치하는 데이터의 index 값을 출력
...
# 합계가 1억이 넘어가는 지점 , 무한루프 사용법
# 1부터 누적합이 1억이 넘어가는 시점 (거짓이 되기 전까지 진행)
result = 0
i = 1
# 무한루프
while(T){
